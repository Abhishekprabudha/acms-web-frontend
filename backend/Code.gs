...
function calculateAllowance_(month, actor) {
  ...
  if (!roster.length) throw new Error('No actual-roster activities were found for '+month+'. Upload a completed template or seed the workbook first.');
  var crewById={}; crew.forEach(function(c){crewById[c.crewId]=c;});
  var payableCrewIds={}; roster.forEach(function(r){if(crewById[r.crewId]) payableCrewIds[r.crewId]=true;});
  var payableCrew=crew.filter(function(c){return payableCrewIds[c.crewId];});
  if (!payableCrew.length) throw new Error('The actual-roster activities do not belong to active crew. No zero-value run was created.');
  ...
  var lines=payableCrew.map(function(c){
  ...
  var total=lines.reduce(function(sum,l){return sum+Number(l.totalAmount);},0);
  if (total <= 0) throw new Error('The activity and effective-dated rate rules produced no payable amount. No zero-value run was created.');
  var run={runId:runId,month:month,importBatchId:batchIds[0]||'',status:'DRAFT',createdAt:new Date().toISOString(),createdBy:actor,checkedAt:'',checkedBy:'',approvedAt:'',approvedBy:'',finalizedAt:'',finalizedBy:'',totalAmount:total.toFixed(2),crewCount:lines.length};
